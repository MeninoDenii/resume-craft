...
export const Onix = ({ data }: BaseResumeProps) => {
  ...
  return (
    <div className="w-full h-full flex">
      <aside className="p-5 w-[250px] h-full relative flex-shrink-0">
        <div className="absolute inset-0 w-full h-full bg-(var:--resume-primary) opacity-40 z-[2]" />
        ...
        <div className="z-[3] relative">
          ...
          <BasicInfos
            infos={infos}
            className="mb-2 flex flex-col gap-1.5 items-start text-xs border border-(var:--resume-primary) px-2.5 py-3"
          />

          {renderSections(sidebarSections, "text-(var:--resume-primary)")}
        </div>
      </aside>

      <section className="p-5 flex-1">{renderSections(mainSections)}</section>
    </div>
  );
};
